...
(function($){
    ...
    $.fn.flumen = function(opt){
        ...
        var is_safari = false; //navigator.userAgent.indexOf("Safari") > -1 && !is_chrome;
        ...
        function init() {
            ...
            if (o.fluid) {
                if (!is_mobile) {
                    $.Kinetic.prototype.t1Init = function(options){
                        if (!$.support.touch) {
                            this.$el
                                .bind('touchstart', this.settings.events.touchStart)
                                .bind('touchend', this.settings.events.inputEnd)
                                .bind('touchmove', this.settings.events.touchMove);
                        }
                    };

                    $slider.kinetic('t1Init', {
                        y: false
                    });
                }
